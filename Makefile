...
.PHONY: go-build
go-build:
	$(eval MAIN_JS := $(shell basename `ls public/p/static/js/main.*.js`))
	$(eval MAIN_CSS := $(shell basename `ls public/p/static/css/main.*.css`))
	$(eval JS := $(shell echo $(MAIN_JS) | cut -d'.' -f2))
	$(eval CSS := $(shell echo $(MAIN_CSS) | cut -d'.' -f2))
	@echo "Set Webui Version: main.$(JS).js main.$(CSS).css"
